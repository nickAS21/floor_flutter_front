...
// 2. АНАЛІЗ ПО КЛІЄНТУ: Працюємо через залізо
      if (httpClient is UsrS100HttpClient) {
        // Якщо це S100 - використовуємо його HTTP API для сканування
        results = await (httpClient as UsrS100HttpClient).getScanResults();
      } else  {
        // Якщо це 232 серія - використовуємо твій UDP провайдер
        results = await provision.scanNetworks(null, httpClient);
      }
      if (mounted) {
  ...
  Widget _buildNetworkSelector() {
    ...
    return DropdownButtonFormField<String>(isExpanded: true, initialValue: hasValue ? selectedSsid : null, isDense: true, decoration: const InputDecoration(labelText: "Available Networks", isDense: true, border: OutlineInputBorder()),
      items: networks.map((n) => DropdownMenuItem<String>(value: n['ssid'].toString(), child: Text("${n['ssid']} (${n['level']}%)", style: const TextStyle(fontSize: 10)))).toList(),
      onChanged: (v) {
        setState(() {
          selectedSsid = v;

...
class _AnalyticsSolarPanelsPageState extends RefreshableState<AnalyticsSolarPanelsPage> {
          ...
          children: [
            ...
            const Padding(
              ...
              child: Row(
                ...
                children: [
                  Text("W", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.orange)),
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  statRow("Solar", last.solarPower, Colors.orange, "W"),
  ...
  Widget _buildMainChart() {
    ...
    // --- 1. РОЗРАХУНОК МАКСИМУМУ ПОТУЖНОСТІ ДЛЯ ПАНЕЛЕЙ (ВАТИ) ---
    ...
    if (maxPowerW <= 0) maxPowerW = 100.0; // Дефолтне значення, якщо даних немає

    // Округлюємо максимум вгору до кратного 50 або 100 для чистої шкали
    ...
    // --- 2. ПОБУДОВА ЛІНІЙ ПАНЕЛЕЙ ПРЯМО У ВАТАХ ---
    for (int i = 0; i < panelKeys.length; i++) {
      final key = panelKeys[i];
      final col = _palette[i % _palette.length];
      lines.add(
              ...
              titlesData: FlTitlesData(
                leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    interval: yInterval,
                    getTitlesWidget: (v, m) {
                      return Text("${v.toInt()}W", style: const TextStyle(fontSize: 8));
                    },
                  ),
                ),
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    showTitles: true,
                    reservedSize: 36, // ЗАПОБІГАЄ ПОМИЛЦІ OVERFLOW ДЛЯ BORDER/TEXT
                    interval: 3600000,
                    getTitlesWidget: (v, m) {
                      final date = DateTime.fromMillisecondsSinceEpoch(v.toInt(), isUtc: true);
